...
int main()
{
    ...
    std::vector<STRUCT> agentVector;

    agentVector.resize(1000);
    glNamedBufferData(agentSSbo,1000*sizeof(STRUCT), NULL, GL_DYNAMIC_COPY);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        else if (spacePressed && glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_RELEASE) 
        {
            ...
            /* glNamedBufferSubData(agentSSbo, TRACER_COUNT*sizeof(soundAgent), TRACER_COUNT*sizeof(soundAgent), &agentVector[0]); */
        }

        // compute shader part

        glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2, agentSSbo);
        computeShader.use_shader(1000,1,1);
        computeShader.wait();
